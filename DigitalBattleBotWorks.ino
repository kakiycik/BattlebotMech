...
const int motorPin1_m1 = 23; // weapin
...
// Map the values from -128 to 128 into the range of 0 to 255
  ...
  int mappedRightx = map(rightx, -128, 128, 256, -258);
...
/////////////////////////////
 ...
 /* if (Ps3.event.button_down.square) {
  ...
  if (Ps3.event.button_down.circle) {
    ...
    digitalWrite(motorPin1_m1, LOW);  // Set direction
    digitalWrite(motorPin2_m1, HIGH); // Motor on
    Serial.println("startw");
  }

  // Stop motor on 'square' button press
  if (Ps3.event.button_down.square) {
    Serial.println("square pressed");
    digitalWrite(motorPin1_m1, LOW);  // Stop motor by setting both pins to LOW
    digitalWrite(motorPin2_m1, LOW);
    Serial.println("stopw");
  }
...
/////////////////////
  ...
  // Initialize the pins
  //pinMode(buzzerPin, OUTPUT); // uncomment!
 ...
 // pinMode(vpPin, INPUT);     // Set the VP pin as an input // uncomment!
  digitalWrite(motorPin1_m1, LOW);  // Stop motor by setting both pins to LOW
  digitalWrite(motorPin2_m1, LOW);
